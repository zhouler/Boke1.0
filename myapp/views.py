...
def mycenter(request):
    ...
    try:
        ...
        return HttpResponse("登录失效，请重新登录")
    if not token:
        return HttpResponse("您没有登录")
    user_object = Token.objects.filter(token=token).first()
    if not user_object:
...
def my_comment(request):
    try:
        ...
        return HttpResponse("登录失效或者您没有登录，请重新登录")
    if not token:
        return HttpResponse("您没有登录")
    user_object = Token.objects.filter(token=token).first()
    if not user_object:
...
def remove_completely_post(request):
    ...
    try:
        Post.objects.filter(false_delete=False).filter(id=del_post_id).delete()
    except:
...
def send(request):      # 需要和signals的触发函数绑定到一起
    try:
...
def  forget_password_two(request):
    email_code=request.POST.get("email_code")#获取ajax发送过来的验证码
